...
def send_log_via_email(
    ...
    """Отправка логов через Resend API"""
    resend_api_key = os.getenv("RESEND_API_KEY")
    
    if not resend_api_key:  
        logger.error("RESEND_API_KEY не установлен в переменных окружения")
        return False
    ...
    try:
        ...
        # Кодируем в base64 для вложения
        file_base64 = base64.b64encode(file_content).decode('utf-8')
        
        # Формируем payload для Resend API
        payload = {
        ...
        logger.info(f"Попытка отправки email с логами на {to_email} через Resend API...")
        resp = requests.post(
        ...
        if resp.status_code == 200 and result.get("id"):
            logger.info(f"✅ Отчет с логами отправлен на почту.  ID: {result. get('id')}")
            return True
        else:
            error_msg = result.get("message", "Неизвестная ошибка")
            logger.error(f"❌ Ошибка при отправке лога на email: {error_msg}")
            print("RESEND ERROR:", error_msg)
            return False
            
    except FileNotFoundError:
        logger. error(f"Error attaching logfile: Файл не найден {log_path}")
        return False
